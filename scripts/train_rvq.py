...
import dac
from dac.model.rvq_autoencoder import SimpleRVQAutoencoder
...
# Model
RVQModel = argbind.bind(SimpleRVQAutoencoder)
...
# Losses (we will use L1 + MultiScaleSTFT, plus VQ losses from the model)
...
def load(
    ...
    if resume:
        ...
        try:
            generator, g_extra = RVQModel.load_from_folder(**kwargs)
        except FileNotFoundError:
            generator, g_extra = None, {}

    generator = RVQModel() if generator is None else generator
...
def val_loop(batch, state, accel):
    ...
    wf = state.waveform_loss(recons, signal)
    stft = state.stft_loss(recons, signal)
...
def checkpoint(state, save_iters, save_path):
    ...
    if state.tracker.is_best("val", "stft/loss"):
